...
- `vuln_funcs`（文字列配列, 任意）: その行についてアドバイザリが指摘する脆弱な
  シンボルの一覧。アドバイザリソース（NVD / GHSA / JVN / OSV — OSV 分は Go vulndb の
  構造化シンボルリスト由来）横断で union し、サーバ側で
  `Pkg.Func` / `Pkg.Type.Method` 形式のセレクタに正規化して返します（trim、末尾の
  `()` 除去、不正形式の除外、重複排除、CVE あたり最大 200 シンボル）。OSV 由来の
  シンボルは**モジュール単位でスコープ**されます: その行のコンポーネントの purl から
  導出した Go モジュールに対して宣言されたシンボルのみが配信され（リストの先頭に
  並びます）、1 つの CVE が複数の Go モジュールにまたがる場合でも、別モジュールの
  シンボルが兄弟コンポーネントの行に混入しません。prose 由来（NVD 等）のシンボルは
  モジュール帰属を持たないため、従来どおり CVE の全行にスコープ済みシンボルの後ろに
  配信されます。行に整形済みシンボルが 1 つもない場合、フィールド自体が**省略**
  されます — その場合 CLI は該当ペアを import-only 解析にフォールバックします。
