...
func TestSbomRepository_Create(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func() {
				mock.ExpectExec("INSERT INTO sboms").
					WithArgs(sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), "cyclonedx", "1.4", sqlmock.AnyArg(), sqlmock.AnyArg()).
		...
		},
		{
			...
			setupMock: func() {
				mock.ExpectExec("INSERT INTO sboms").
					WithArgs(sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), "spdx", "2.3", sqlmock.AnyArg(), sqlmock.AnyArg()).
		...
		},
		{
			...
			setupMock: func() {
				mock.ExpectExec("INSERT INTO sboms").
					WithArgs(sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), "cyclonedx", "1.5", sqlmock.AnyArg(), sqlmock.AnyArg()).
...
// TestSbomRepository_Create_PassesTenantID asserts that the tenant_id column
...
func TestSbomRepository_Create_PassesTenantID(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("failed to create mock db: %v", err)
	}
	...
	mock.ExpectExec("INSERT INTO sboms").
		WithArgs(sbomID, tenantID, projectID, "cyclonedx", "1.4", sqlmock.AnyArg(), createdAt).
		WillReturnResult(sqlmock.NewResult(1, 1))

	err = repo.Create(context.Background(), &model.Sbom{
		ID:        sbomID,
		TenantID:  tenantID,
		ProjectID: projectID,
		Format:    "cyclonedx",
...
func TestSbomRepository_LookupProjectTenantID(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("failed to create mock db: %v", err)
	}
