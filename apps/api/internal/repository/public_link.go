...
// public_links / public_link_access_logs both enforce tenant-scoped RLS
// (migration 009), so the dashboard list/view endpoints need the tenant GUC
// set by TenantTx to return rows (codex-r1 Finding 2). Token-lookup callers
// (anonymous access, no tenant context yet) keep getting raw r.db via the
// fallback because their ctx carries no tx.
...
func (r *PublicLinkRepository) IsDownloadLimitReached(ctx context.Context, id uuid.UUID) (bool, error) {
	...
	var downloaded int
	if err := r.q(ctx).QueryRowContext(ctx, query, id).Scan(&allowed, &downloaded); err != nil {
		return false, err
	}
	if !allowed.Valid {
		return false, nil
...
func (r *PublicLinkRepository) UpdateCounts(ctx context.Context, id uuid.UUID, viewCount, downloadCount int) error {
	...
	_, err := r.q(ctx).ExecContext(ctx, query, viewCount, downloadCount, id)
	return err
}
...
func (r *PublicLinkRepository) Touch(ctx context.Context, id uuid.UUID, updatedAt time.Time) error {
	...
	_, err := r.q(ctx).ExecContext(ctx, query, updatedAt, id)
	return err
}
