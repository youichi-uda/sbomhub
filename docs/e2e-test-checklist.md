...
| AUTH-03 | 未認証アクセス | ログインページにリダイレクト | OK | dashboard/projects/settings全てリダイレクト確認 |
...
| PROJ-08 | プロジェクト削除 | プロジェクト削除可能 | OK | 修正済み（BUG-04解消） |
...
| COMP-02 | 検索/フィルター | 検索機能動作 | OK | 横断検索で確認 |
| COMP-03 | ライセンス表示 | ライセンス情報表示 | OK | ライセンス列表示確認 |
...
| SETT-03 | APIキー作成 | 新規キー作成可能 | OK | 修正済み（BUG-05解消） |
...
| NAV-03 | 組織切り替え | 組織セレクター動作 | OK | "test github"組織表示、切替ボタンあり |
...
| BUG-03 | 外部連携API 401エラー | /api/v1/integrations が401を返す | ✅ **修正済み** - settingsフォルダを(dashboard)/配下に移動 |
| BUG-04 | プロジェクト削除時JSエラー | 削除APIは成功するがフロントエンドでSyntaxError | ✅ **修正済み** - 204 No Content対応 |
| BUG-05 | APIキー管理で401エラー | /api/v1/apikeys が401を返す | ✅ **修正済み** - settingsフォルダを(dashboard)/配下に移動 |
...
- ✅ BUG-03: settingsフォルダを(dashboard)/配下に移動（認証プロバイダ継承）
- ✅ BUG-04: api.tsで204 No Content/空レスポンス対応
- ✅ BUG-05: settingsフォルダを(dashboard)/配下に移動（認証プロバイダ継承）
...
5. **apps/web/src/lib/api.ts** (BUG-04修正)
   - HTTP 204 No Contentレスポンス対応
   - 空レスポンス（content-length: 0）対応
   - res.text()で先にテキスト取得し、空でない場合のみJSON.parse()

6. **apps/web/src/app/[locale]/settings/** (BUG-03, BUG-05修正)
   - settingsフォルダ全体を `[locale]/(dashboard)/settings/` に移動
   - これにより(dashboard)/layout.tsxのAuthProvider継承が有効化
   - 対象ページ: apikeys, integrations, ipa, reports, scan, sso
