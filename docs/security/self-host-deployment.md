...
最初に全 path 共通で operational scripts を配置する。
...
次に install path に合わせて secret source を選ぶ。cron / systemd timer 例は
`/opt/sbomhub/docker/secrets/postgres_app_password.txt` と
`/opt/sbomhub/docker/secrets/encryption_key.txt` を読むため、どちらか一方を完了してから進む。

**Path A: Standard OSS install (`.env` ベース)**

標準の OSS `./install.sh --start` / cron 経路では `docker/secrets/*` は生成されず、
`.env` が secret source になる。この場合は `.env` から wrapper 用の最小 secret file だけを
作る。
...
printf '%s' "$ENCRYPTION_KEY" | sudo install -m 600 -o sbomhub -g sbomhub /dev/stdin \
...
`docker compose -f docker-compose.enterprise.yml` 経由で運用しており、
`docker/secrets/postgres_app_password.txt` と `docker/secrets/encryption_key.txt` が既に存在する場合は、
既存の Docker secrets を `/opt` 側へ同期する。

```bash
sudo install -d -m 700 -o sbomhub -g sbomhub /opt/sbomhub/docker/secrets

for secret in postgres_app_password.txt encryption_key.txt; do
  sudo install -m 600 -o sbomhub -g sbomhub \
    docker/secrets/"$secret" /opt/sbomhub/docker/secrets/"$secret"
done
```
...
copy the existing `docker/secrets/postgres_app_password.txt` and
`docker/secrets/encryption_key.txt` files into `/opt/sbomhub`. This keeps
unattended jobs independent from a developer checkout path while preserving the
boundary between standard OSS and Enterprise secret handling.
...
cron 例を入れる前に、§4.5 の `/opt/sbomhub` deployment で Path A (Standard OSS `.env`) または
Path B (Enterprise Docker secrets) のどちらかを完了しておくこと。
...
例を入れる前に、§4.5 の `/opt/sbomhub` deployment で Path A (Standard OSS `.env`) または
Path B (Enterprise Docker secrets) のどちらかを完了しておくこと。 systemd では
...
cron 例を入れる前に、§4.5 の `/opt/sbomhub` deployment で Path A (Standard OSS `.env`) または
Path B (Enterprise Docker secrets) のどちらかを完了しておくこと。これにより
...
### 9.7 Disaster Recovery rehearsal (`dr-rehearsal.sh`)

backup を取るだけでは「実際に復元できる」 ことは保証されない。production deploy 前、および
**四半期に 1 回** の staging 訓練では、M7-4 の DR rehearsal script を実行する。

```bash
./docker/scripts/dr-rehearsal.sh --verbose
```

この script は ephemeral compose 環境で以下を end-to-end に確認する。
...
debug 時は `--keep-env` で cleanup を skip し、出力された compose project / work dir を使って
手動確認できる。通常実行では EXIT trap で `docker compose down -v` まで行うため、訓練用 volume
は残らない。

CI nightly での自動実行は `.github/workflows/dr-rehearsal.yml` に weekly Sunday 04:00 UTC
schedule と manual trigger を用意している。GitHub Actions runner 上の full compose + Go build は
重めなので、必要に応じて `workflow_dispatch` のみへ落とす。

従来の手動 restore 訓練で発見しがちな問題:
