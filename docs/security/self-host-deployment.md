...
- M4-6 (本ガイド執筆時点では未マージ) で `docker-compose.enterprise.yml` が追加される予定。
  これは Ollama (Local LLM) を 1 ファイルで同梱する enterprise 向け変種。

  > **TODO**: M4-6 完了後、 `docker-compose.enterprise.yml` への relative link と
  > 起動手順 (`docker compose -f docker-compose.yml -f docker-compose.enterprise.yml up -d`) を
  > §11 に追記する。
...
| ロール | 役割 | 権限 | 接続経路 |
|---|---|---|---|
| `sbomhub_migrator` | スキーマ migration (DDL 適用) を行う一時接続 | `CREATEDB`, `CREATEROLE`, テーブル所有者、 RLS bypass **なし** | `MIGRATE_DATABASE_URL` (起動時のみ open → migration 後 close) |
| `sbomhub_app` | アプリ runtime の全クエリを行う長期接続 | `SELECT/INSERT/UPDATE/DELETE` のみ、 **`NOBYPASSRLS` 必須** | `DATABASE_URL` |
| `sbomhub` (POSTGRES_USER) | DB owner / superuser-equivalent | 上記 2 ロールの bootstrap にのみ使う、 runtime では使わない | 通常は使わない (install.sh が docker compose exec 経由でのみ利用) |
...
`assertAppRoleNotBypassRLS` が起動時に「`current_user` が `BYPASSRLS=false`」 を確認する。
production / staging で BYPASSRLS が true だと **起動を拒否** する (development だけ warning に
downgrade)。
...
これは **即時修正対象** (重大インシデント)。 multi-tenant 分離が完全に崩壊している状態で、
他テナントの SBOM が見える可能性がある。
...
その後 API を再起動し、 起動 log に `DB role check passed role=sbomhub_app bypass_rls=false` が
出ることを確認する (`apps/api/cmd/server/main.go` の `assertAppRoleNotBypassRLS`)。
...
> **TODO**: 現在の `apps/api` は `*_FILE` パターンを直接サポートしていない。
> entrypoint script で `ENCRYPTION_KEY=$(cat $ENCRYPTION_KEY_FILE) exec /sbomhub-api` の形で
> ラップするか、 別 issue で本体に `*_FILE` サポートを入れる。
...
| ollama (M4-6 enterprise) | 11434 | **localhost-only bind 推奨** | 外部からの prompt 投入 / model abuse 防止 |
...
```yaml
# docker-compose.enterprise.yml (M4-6 想定)
services:
  ollama:
...
> **TODO**: M4-6 で `docker/scripts/backup.sh` が提供される予定 (本ガイド執筆時点では未存在)。
> 単純な cron + 上記コマンドの組み合わせで暫定運用可能。 M4-6 完了後、 公式 script への link を
> 本節に追記する。

```cron
# /etc/cron.d/sbomhub-backup (毎日 02:00 JST 実行)
0 2 * * *  sbomhub  /opt/sbomhub/backup.sh >> /var/log/sbomhub-backup.log 2>&1
```
...
#   DB role check passed role=sbomhub_app bypass_rls=false
```
...
M4-6 で追加予定の `docker-compose.enterprise.yml` が、 ollama service を 1 ファイルで上乗せする。
起動:
...
> **TODO**: M4-6 完了後、 `docker-compose.enterprise.yml` の relative link と GPU passthrough
> 設定例 (`devices: [/dev/dri], runtime: nvidia` 等) を本節に追記する。
