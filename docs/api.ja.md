...
存在確認のプローブには使えません。例外はプロジェクト一覧 2 本で、これらは拒否ではなく
**キー自身のプロジェクトだけに絞り込んだ 1 件の一覧**を返します。一覧の場合、応答本文
そのものが「絞り込まれる集合」であり、どのフィールドの意味も変わらないためです。
プロジェクト限定キーが使える 34 エンドポイント
(パスでプロジェクトを指定する 29 + 指定しない 5) と使えない 4 エンドポイントの
...
`GET /api/v1/projects` は**含まれません**。機械クライアントは代わりに
`GET /api/v1/cli/projects` を使用してください — テナント単位キーはテナント配下の全
プロジェクトを、プロジェクト限定キーは自分のプロジェクト 1 件だけを取得します。
