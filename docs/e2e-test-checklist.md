...
| PROJ-02 | 一覧件数 | 正しい件数が表示 | **注意** | ダッシュボード13件、一覧10件の不一致あり |
...
| PROJ-08 | プロジェクト削除 | プロジェクト削除可能 | N/A | 実データ影響を避けてスキップ |
...
| VULN-03 | CVE詳細リンク | CVE詳細ページへ遷移 | **NG** | Vulnerability Test Projectへのリンクが404 |
...
| INTG-04 | 連携タイプ選択 | Jira/Backlog選択可能 | N/A | ダイアログ内は未確認 |
...
| SETT-03 | APIキー作成 | 新規キー作成可能 | N/A | 実データ影響を避けてスキップ |
...
| ID | 問題 | 詳細 | 影響 |
|----|------|------|------|
| BUG-01 | ダッシュボードからのプロジェクトリンクが404 | "Vulnerability Test Project"へのリンクが「プロジェクトが見つかりません」エラー | ダッシュボードに表示される脆弱性情報へのアクセス不可 |

### 重大度: 中
| ID | 問題 | 詳細 | 影響 |
|----|------|------|------|
| BUG-02 | プロジェクト数の不一致 | ダッシュボード表示13件、プロジェクト一覧表示10件 | データ整合性の問題 |
| BUG-03 | 外部連携API 401エラー | /api/v1/integrations が401を返す | 連携一覧取得に失敗（UI表示は正常） |

### 重大度: 低
| ID | 問題 | 詳細 | 影響 |
|----|------|------|------|
| OBS-01 | ブレッドクラム未実装 | パンくずリスト表示なし | ナビゲーション利便性 |
...
- **全体評価**: 概ね良好
- **合格率**: 85.2% (46/54項目OK)
- **重大な問題**: 1件（ダッシュボードからのプロジェクトリンク404）
- **推奨アクション**:
  1. BUG-01のデータ不整合を調査・修正
  2. BUG-02のプロジェクト数不一致を調査
  3. BUG-03の外部連携API認証を確認
