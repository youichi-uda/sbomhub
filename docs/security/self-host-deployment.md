...
| シナリオ | コマンド |
|---|---|
| restore 直後の自動 smoke (Step 8) | `VERIFY_ENCRYPTION=1 VERIFY_DB_URL="$DATABASE_URL" ./scripts/restore.sh backup.tar.gz` |
| 手動 spot check (BYOK LLM key カラム) | `export DATABASE_URL="postgres://sbomhub_app:...@127.0.0.1:5432/sbomhub?sslmode=disable"; ENCRYPTION_KEY="$(cat docker/secrets/encryption_key.txt)" ./scripts/verify-encryption.sh` |
| 手動 spot check (key file 経由) | `export DATABASE_URL="postgres://sbomhub_app:...@127.0.0.1:5432/sbomhub?sslmode=disable"; ./scripts/verify-encryption.sh --key-file docker/secrets/encryption_key.txt` |
| 手動 spot check (issue tracker token) | `export DATABASE_URL="postgres://sbomhub_app:...@127.0.0.1:5432/sbomhub?sslmode=disable"; ENCRYPTION_KEY="$(cat docker/secrets/encryption_key.txt)" ./scripts/verify-encryption.sh --table issue_tracker_connections --column auth_token_encrypted` |
...
- ENCRYPTION_KEY は env (`ENCRYPTION_KEY=...`) または `--key-file <path>` で渡す。 argv
  (`--key ...`) は `/proc/<pid>/cmdline` / `ps` 経由で見えやすいため deprecated。 script は
  どちらの推奨経路でも env に正規化してから Go binary に渡す。
