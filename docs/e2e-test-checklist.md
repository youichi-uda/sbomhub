...
| AUDIT-02 | **プラン機能アクセス** | **Cloud Teamプランで監査ログ機能が利用可能か** | **監査ログ一覧が表示される（プラン制限メッセージは表示されない）** | **NG** | **BUG-06: Cloud Teamなのに「Pro版以上のプランでご利用いただけます」と表示される** |
...
| ID | 問題 | 詳細 | ステータス |
|----|------|------|------------|
| BUG-06 | 監査ログがCloud Teamプランで利用不可 | Cloud Team（¥20,000/月）契約中だが「Pro版以上のプランでご利用いただけます」と表示される。**原因特定**: `migrations/008_subscriptions.up.sql` の `plan_limits` テーブルに `audit_logs` フィーチャーが未登録。修正マイグレーション `024_fix_plan_limits_audit_logs.up.sql` を作成済み | 🟡 **修正作成済み・デプロイ待ち** |
...
- **全体評価**: 不合格（バグあり）
- **検出バグ**: 1件（BUG-06: 監査ログプラン判定エラー）
- **最終試験日**: 2026-02-01

### 要対応
- 🟡 BUG-06: 監査ログのプラン判定ロジック修正
  - **原因**: `migrations/008_subscriptions.up.sql` で `plan_limits` テーブルに `audit_logs` 機能が登録されていなかった
  - **修正**: `migrations/024_fix_plan_limits_audit_logs.up.sql` を作成
  - **次のアクション**: 本番環境にマイグレーションをデプロイ
