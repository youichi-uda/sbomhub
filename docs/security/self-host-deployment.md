...
| 手動 spot check (BYOK LLM key カラム) | `ENCRYPTION_KEY="$(cat docker/secrets/encryption_key.txt)" ./scripts/verify-encryption.sh --db-url "$DATABASE_URL"` |
| 手動 spot check (key file 経由) | `./scripts/verify-encryption.sh --key-file docker/secrets/encryption_key.txt --db-url "$DATABASE_URL"` |
| 手動 spot check (issue tracker token) | `ENCRYPTION_KEY="$(cat docker/secrets/encryption_key.txt)" ./scripts/verify-encryption.sh --db-url "$DATABASE_URL" --table issue_tracker_connections --column auth_token_encrypted` |
