...
で定義しています。 SBOMHub は `ENCRYPTION_KEY` 文字列の先頭 32 raw byte をそのまま
AES key として使い、 `openssl rand -base64 32` の出力を base64 decode しません。
...
| `issue_tracker_connections` | `auth_token_encrypted` | TEXT base64 `nonce \|\| sealed` | 連携先 issue tracker のトークン — Jira / Backlog API トークン or GitHub PAT (テナント別) | `internal/service/issue_tracker.go` | **はい** |
...
host shell で値を export した上で、 compose には `-e VAR` (名前のみ) で渡す。
`-e VAR=value` は展開済み secret が host 側の `docker compose` process argv に
載るため使わない。 enterprise compose の `sbomhub-api` は通常 entrypoint wrapper
内で Docker secrets から `DATABASE_URL` を組み立てるが、 `--entrypoint
/usr/local/bin/migrate-encryption` はその wrapper を bypass するため、 caller が
`DATABASE_URL` を明示的に用意する必要がある。 `--db-url` flag は backward
compatibility のため残っているが、 DSN を container argv に載せないため本
runbook では使わない。

> **警告:** Docker secrets の password は base64 生成時に `/`, `+`, `=`,
> `@`, `:` などの特殊文字を含み得る。 `postgres://` DSN に埋め込む前に
> URL encode しないと接続に失敗する可能性がある。
...
- 中断時は同じ dry-run report で再実行できる。 厳密に再開したい場合は
  最後に処理した report row の `resume_token` を
  `--resume-from <resume_token>` に渡す。 resume は token 内の単一 tenant と、
  現在指定した単一 `--table` / `--column` に限定される。 default の複数 target
  run では使わない。 token には dry-run report file content 由来の HMAC 署名が
  含まれるため、同じ `--report-input` と組み合わせて使う。
...
3. **Issue tracker 連携** — Jira / Backlog / GitHub Issues 連携を設定して
   いた任意のテナントの統合ページを開く。 connection が active と表示されること。
...
3. 影響テナントへ BYOK LLM API key と issue tracker 連携 (Jira / Backlog
   API トークン、 GitHub PAT) を設定画面 /
...
| インシデント (鍵漏洩) | 即時 | 全 BYOK LLM API key と `issue_tracker_connections` トークンを露出済みとして扱う; マスター鍵ローテーションは流出済み平文を無効化しない。 ローテーション後、 影響テナントには上流の issue tracker credential (Jira / Backlog API トークン、 GitHub PAT) と LLM provider key も回すよう案内。 |
